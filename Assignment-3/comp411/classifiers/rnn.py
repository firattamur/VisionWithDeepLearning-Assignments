...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        hiddens, cache = rnn_forward(x, h0, Wx, Wh, b)

        out, cache_affine = temporal_affine_forward(hiddens, W_vocab, b_vocab)
    ...
    def sample_greedily(self, features, max_length=30):
        ...
        V_emb, W_emb = W_embed.shape
        x = np.ones((N, W_emb)) * W_embed[self._start]
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            indices = np.random.choice(out.shape[1], 1, p=probs.flatten())
            captions[:, i] = indices
            x = W_embed[indices]
            h_init = next_h
